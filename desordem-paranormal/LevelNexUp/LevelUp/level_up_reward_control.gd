...
var current_items: Array[Resource]
var erasing: bool
...
func get_array_of_items() -> void:
	...
	for i in range(3):
		current_items.append(items_got[i].values()[0])
	
	insert_items_into_slots()
	

func insert_items_into_slots() -> void:
	slot_1.item = current_items[0]
	slot_2.item = current_items[1]
	slot_3.item = current_items[2]
	slot_1.update_slot()
	slot_2.update_slot()
	slot_3.update_slot()
...
func seleted_item(item: Resource) -> void:
	if erasing:
		...
		if current_items[0] == item:
			slot_1.item_deleted()
		elif current_items[1] == item:
			slot_2.item_deleted()
		elif current_items[2] == item:
			slot_3.item_deleted()
